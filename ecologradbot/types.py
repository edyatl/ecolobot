...
# import ecologradbot.util as util
### util block ###
# util.is_dict
def util_is_dict(var):
    return isinstance(var, dict)


# util.is_string
def util_is_string(var):
    return isinstance(var, str)


# util.chunks
def util_chunks(lst, n):
...
# util.is_bytes
def util_is_bytes(var):
    return isinstance(var, bytes)


# util.generate_random_token
def util_generate_random_token():
    # return ''.join(random.sample(string.ascii_letters, 16))
    return secrets.token_urlsafe(12)


# util.is_pil_image
def util_is_pil_image(var):
    return pil_imported and isinstance(var, Image.Image)


# util.pil_image_to_file
def util_pil_image_to_file(image, extension='JPEG', quality='web_low'):
    if pil_imported:
        photoBuffer = BytesIO()
...
class JsonDeserializable(object):
    ...
    def check_json(json_type):
        ...
        :return:
        """
        if util_is_dict(json_type):
            return json_type
        elif util_is_string(json_type):
...
class ReplyKeyboardMarkup(JsonSerializable):
    ...
    def add(self, *args, row_width=None):
        ...
        for row in util_chunks(args, row_width):
            button_array = []
            for button in row:
                if util_is_string(button):
                    button_array.append({'text': button})
                elif util_is_bytes(button):
...
class InputMedia(Dictionaryable, JsonSerializable):
    def __init__(self, type, media, caption=None, parse_mode=None):
        ...
        if util_is_string(self.media):
            self._media_name = ''
            self._media_dic = self.media
        else:
            self._media_name = util_generate_random_token()
...
class InputMediaPhoto(InputMedia):
    def __init__(self, media, caption=None, parse_mode=None):
        if util_is_pil_image(media):
            media = util_pil_image_to_file(media)
